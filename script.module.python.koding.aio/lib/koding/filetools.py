...
find this problem is still occuring please let me know (Made by Candy on noobsandnerds.com forum), thankyou.
...
leave blank. If left blank and the platform is Made by Candy or LibreELEC
...
# If it's LE/OE and there are no files to ignore we do a hard reset
    ...
    if ( len(ignore)==0 ) and ( len(keep_addons)==0 ) and ( xbmc.getCondVisibility("System.HasAddon(service.libreelec.settings)") or xbmc.getCondVisibility("System.HasAddon(service.Made by Candy.settings)") ):
