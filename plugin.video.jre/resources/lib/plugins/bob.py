...
class BoB(Plugin):
    ...
    def get_link_message(self, *args):
        messages = [
            ...
            {
                'HD': 'Youraddonname\'s Ya Uncle',
                'SD': 'Youraddonname\'s NOT Ya Cousin'
            },
            {
    ...
    def get_searching_message(self, preset):
        ...
        if xbmcaddon.Addon().getSetting('enable_offensive') == "true":
            messages.extend([
                ...
                'Frankly my dear, I don\'t give a Youraddonname',
                'Beast Build Detected, Installing dangerous pyo file',
                'Costa wants to aduse Emma'
            ])

        if preset == "search":
            messages.extend(['Youraddonname is popping in Blu Ray Disc'])
        elif preset == "searchsd":
            messages.extend([
                'Youraddonname rummaging through his vhs collection',
            ])
